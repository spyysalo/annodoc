    ...
    var createDocumentationLink = function(e, collections) {
        ...
        if (collDocByName[s] !== null) {
            var coll = collDocByName[s][0],
                ...
                collDoc = coll + '/' + doc.title,
                url = doc.url;

            // allow in-document auto-linking targets to pre-empt
            // links to other documents
            console.log(destByCollDoc, collDoc, destByCollDoc[collDoc]);
            if (destByCollDoc[collDoc] !== undefined) {
                url = '#' + destByCollDoc[collDoc].id;
            }
