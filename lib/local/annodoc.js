...
//         		      function(a, b) { return b.replace(/\s+/g, ' '); });
    ...
    // represents CoNLL-U document
    var ConllUDocument = function() {
        this.reset();
    };

    ConllUDocument.prototype.reset = function() {
        this.sentences = [];
        this.error = false;
    ...
    ConllUDocument.prototype.log = function(message) {
        this.logger(message);
    };

    ConllUDocument.prototype.logError = function(message) {
        this.log('error: ' + message);
        this.error = true;
    };

    ConllUDocument.prototype.parse = function(input, logger, strict) {
        ...
        var splitter = selectConllUSplitter(input, this.logger, strict);
        ...
        for (var idx=0; idx<lines.length; idx++) {
            ...
            if (fields.length !== 10) {
                ...
                repairConllUFields(fields, this.logger);
            }
    ...
    ConllUDocument.prototype.toBrat = function(logger) {
        if (logger !== undefined) {
            this.logger = logger;
        }
