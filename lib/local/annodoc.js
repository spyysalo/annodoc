...
//                    '</span></span>';
    ...
    var embedAnnotation = function(elem, parse, cdata, data, options) {
        ...
        elem.attr('embeddedSequenceNum', myIdSeq);
        ...
        // is empty, use sequence number as label for first sentence.
        if (parsed.sentlabels === undefined || parsed.sentlabels.length === 0) {
            parsed.sentlabels = ['' + myIdSeq];
        } else if (parsed.sentlabels[0] === null) {
            parsed.sentlabels[0] = '' + myIdSeq;
        }
    ...
    var createDocumentationLink = function(e, collections) {
        ...
        if (collDocByName[s] !== null) {
            ...
            // if the link was set by the element text content, turn
            // e.g. "entity/person" into "person" in doc link text
            if (linkedByText && (m = e.text().match(/^.+\/(.+)$/)) !== null) {
                e.text(m[1]);
            }
        } else {
    ...
    var docLinkRegex = /^((?:[a-zA-Z_-]+\/)*)([a-zA-Z0-9_-]+)$/;

    var createDocumentationLinks = function(collections) {
        // find anchors that either 1) have empty or missing href, id
        // and name attributes, or 2) have a href that matches
        // docLinkRegex, and turn these into links into the
        // documentation.
        var linkable = function(e) {
