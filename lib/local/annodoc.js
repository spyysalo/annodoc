...
//                    '</span></span>';
    ...
    var embedAnnotation = function(elem, parse, cdata, data, options) {
        ...
        var assignedIdSeq = embeddedIdSeq,
            eId = 'embedded-' + embeddedIdSeq++,
            inputTabId = eId + '-2',
            bratTabId = eId + '-3',
            infoTabId = eId + '-4';
        ...
        dispatcher.on('doneRendering', function() {
            // Make the first sentence number match the assigned ID
            // sequence number, and display it. Other sentence numbers
            // remain hidden.
            var firstSentNumText = visDiv.find('.sentnum text:first');
            firstSentNumText.text(assignedIdSeq);
            firstSentNumText.show();
