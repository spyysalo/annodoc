...
//                    '</span></span>';
    ...
    // helper for activate(): generate embedded visualizations
    var createEmbeddedVisualizations = function(collectionData) {
        // call embedAnnotation with the appropriate parsing function
        // for each element with a class found in parseFunctionMap
        var embedClasses = Object.keys(parseFunctionMap);
        $(embedClasses.join(',')).each(function(idx) {            
            var parseFunction = findParseFunction(this);
            embedAnnotation($(this), parseFunction, collectionData);
        });
    };
